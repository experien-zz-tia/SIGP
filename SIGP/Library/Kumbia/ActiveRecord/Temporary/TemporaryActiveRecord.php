 ...
 * @license		New BSD License
 */
class TemporaryActiveRecord extends ActiveRecord {
	...
	public function __construct(){
		if(Facility::getFacility()==Facility::USER_LEVEL){
			...
			if(EntityManager::existsTemporaryEntity($tableName)==false){
	 ...
	 * @param DbBase $db
	 */
	private function _createTemporaryTable($tableName, $db=null){
	...
	protected function _dumpInfo($tableName, $schemaName=''){
		...
		if($db->temporaryTableExists(strtolower($tableName), $schemaName)==false){
			$this->_createTemporaryTable($tableName, $db);
		}
	}
