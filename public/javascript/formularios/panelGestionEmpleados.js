panelGestionEmpleados = Ext.extend(panelGestionEmpleadosUi, {
	...
	modificar : function() {
		...
		if (!index) {
			...
			var comboDecanato = Ext.getCmp('cmbDecanato');
			var storeDecanato = comboDecanato.getStore();
			storeDecanato.load({
						callback : function() {
							comboDecanato.setValue(index.get('decanatoId'));
						}
					});
	...
	eliminar : function() {
		...
		if (!index) {
			...
			Ext.Msg
					.confirm(
							'Confirmaci&oacute;n',
							'&iquest; Est&aacute; seguro de eliminar el empleado seleccionado?',
							function(btn) {
								if (btn === 'yes') {
									Ext.Ajax.request({
										...
										success : function(respuesta, request) {
											...
											if (jsonData.success == true) {
												...
												Ext.MessageBox.show({
													title : 'Error.',
													msg : 'No se ha eliminado el empleado:<BR>'
															+ jsonData.errorMsj,
													buttons : Ext.MessageBox.OK,
